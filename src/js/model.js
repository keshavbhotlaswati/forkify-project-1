...
const createRecipeObject=function(){
  ...
  return {
    ...
    image: recipe.image_url,
    servings: recipe.servings,
    cookingTime: recipe.cookikng_url,
    ingredients: recipe.ingredients,
    ... (recipe.key && {key:recipe.key}),
...
export const addBookmark=function(recipe){
  //add bookmark
state.bookmarks.push(recipe);
//markk currect recipe as boomark
...
export const deleteBookmark=function(id){
  ...
  const index=state.bookmarks.findIndex(el=>el.id===id);
state.bookmarks.splice(index,1);
//mark currect book marked as false property  
...
export const uploadRecipe=async function(newRecipe){
  try{
  console.log('ingredients'+ Object.entries(newRecipe));
const ingredients=Object.entries(newRecipe).filter(entry=>entry[0].startsWith('ingrediemt') && entry[1]!='').map(ing=>{
const ingArr=ing[1].split(',').map(el=>el.trim());
